...
module.exports = function openDatabaseConnection (callback) {
	...
	if (keystone.get('mongo replica set')) {
		...
		keystone.mongoose.set('strictQuery', false);
		console.log('connecting to mongo2');
		...
		keystone.mongoose.set('strictQuery', false);
		console.log('connecting to mongo1');
		console.log(keystone.get('mongo'));
		console.log(mongo_options);
	...
	keystone.mongoose.connection.on('error', function (err) {
		...
		var connected = function () {
			if (keystone.get('auto update')) {
				...
				console.log('applying auto update');
